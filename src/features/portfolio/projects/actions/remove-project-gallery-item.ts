...
import { detachMediaGalleryItem } from '@/features/media/lib/media-attachments'
import type { MediaGalleryItem } from '@/features/media/types'
import { removeProjectGalleryItemSchema } from '../schemas/project-gallery.schema'
import { syncProjectGalleryJson } from '../lib/project-gallery'

export async function removeProjectGalleryItem(input: unknown): Promise<MutationResult<MediaGalleryItem[]>> {
  await assertAdminAccess()

  return runMutation(removeProjectGalleryItemSchema, input, async (data) => {
    ...
    await detachMediaGalleryItem(data.attachmentId)
